...
SafetyLimitPosition=1500; %% INITIAL MOTOR POSIZYONUNDAN MAXIMUM UZAKLAÞABÝLÝNECEK DEÐER.

revised_current=0;



%% Kayýt yapýlacak vectorlerin boyutlarýný baþtan belirleyelim, böylece real-time performansý artýralým
%Baþlangýç vektör boyutlarýný, saniyede 2000 veri olacak þekilde atadým, bu
%daha az olacaktýr, daha sonra düzenlenir.
vel = -99*ones(1,T*50);
...
tic; %MOTOR HAREKET KODU BAÞLAYINCA BÝR TÝMER SAYMAYA BAÞLIYOR.
                           ...
                           while(t<T)
                                    ...
                                    %eference_position = (pi/4)*sin(2*pi*0.2*t);
                                    ...
                                    posValue = double(typecast(uint32(dxl_present_position), 'int32') - initialMotorPosition); % MeasuredPos-InitialPos demiþ olduk.
                                    %posValue = posValue/4;
                                    
                                    posValueSafety = typecast(uint32(dxl_present_position), 'int32') - initialMotorPosition; %Bunu sadece safety check için kullanýyorum. 
                                    ...
                                    vel(index) = (2*pi/60)*0.229*double(typecast(uint32(dxl_present_vel), 'int32')); %her iterasyonda pozisyon bilgisini vel vektörüne atýyoruz atýyoruz.                                   
...
%                                    DerivativeAlternative=gradient(ref(1:index),TimeVector(1:index));
%                                    DerivativeAlternative = DerivativeAlternative(end)- vel(index);
%                                    DerivativeAlternativeVector(index) =   DerivativeAlternative;  


                                    %Assign revised current
                                    revised_current = ((((9*error) + (2*error_int) + (0.3*error_der))));                                       
                                    ...
                                    %pause(0.01)
                                    %pause(abs(1/double(SamplingFreq)));
...
%% Vektörler içindeki kullanýlmamýþ, -99 deðerleri silinecek böylece sadece önemli veriler kalacaktýr.
...
PresentCurrent(find(PresentCurrent==-99))=[];
...
plot(vel)
%hold on
%plot(DerivativeAlternativeVector)
legend('derivative','velocity')

% figure
% plot(TimeVector(2:end),PosError)
% hold on
% plot(TimeVector(2:end),PosError_int)
% hold on
% plot(TimeVector(2:end),PosError_der)
% legend('PosError','IntError','DerError')
